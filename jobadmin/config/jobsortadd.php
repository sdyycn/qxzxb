...
function getbasesort($db){
	$result=$db->query("select jobsortid,sortname from `ejob_jobsort` where level = '0'");
	$basesort = array();
...
//保存行业类别
function saveJobsort($db, $table, $data) {
...
//获取数据库中最大序号值
if($_GET["act"]=="addsave"){
	$result=$db->query("select max(xh) from `ejob_jobsort`");
	if  ($db->result($result)){
	...
	$table = "ejob_jobsort";
	if(saveJobsort($db, $table, $data)){
		alert("添加成功!","jobsort.php");
		}
	else{
		alert("添加失败!","jobsort.php");
		}
}
?>
...
{
		?>
            <option value="<?php echo $basesort[$i]["jobsortid"]?>"><?php echo $basesort[$i]["sortname"]?></option>
            <?php
			}
        ?>
          </select></td>
