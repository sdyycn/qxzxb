...
class NewsViewPage extends AdminPage{

	function __construct(){
		parent::__construct('news_view.html');
	}
	function display(){
		...
		$newsid = $_GET['id'];
		$sql = "SELECT newstitle as title, pic, newssortid as id, author, source, newscontent as content FROM $t WHERE newsid = $newsid";
		$stmt = $pdo->query($sql);
		$news = array();
		$stmt->bindColumn(1, $news['title'], PDO::PARAM_STR, 100);
		$stmt->bindColumn(2, $news['pic'], PDO::PARAM_STR, 200);
		$stmt->bindColumn(3, $news['id'], PDO::PARAM_INT);
		...
		$this->smarty->assign('select_id', $news['id']);
