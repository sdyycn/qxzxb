...
function form_fields($id,$path='',$edit='false'){
	...
	$opt='';
		foreach($field as $key=>$value){
			...
			if($value['form_id']==$id){
				...
				if(!in_array($value['field_type'],array('checkbox','radio'))){
				$help=empty($value['info'])?"":"<span title=\"{$value['field_info']}\" class=\"help\"></span>";
				$opt.="<tr><td class=\"w1\" style=\"width:20%\">{$help}<label>{$value['use_name']}:</label></td><td style=\"width:80%\">".$value['field_type']($value['field_name'],$v)."{$note}</td></tr>";
				}else{
				if($edit=='false'){$v='';}
				$opt.="<tr><td class=\"w1\" style=\"width:20%\"><span title=\"{$value['field_info']}\" class=\"help\"></span>{$value['use_name']}:</td><td style=\"width:80%\">".$value['field_type']($value['field_name'],$value['field_value'],$v)."{$note}</td></tr>";
				}
			}
		}
	...
	$str.=$js;
	$str.="}</script>";
	$str.="<form action=\"{$path}plus/form.php\" class=\"form_form\" name=\"form_form\" method=\"post\" onsubmit=\"return check_form();\">";
	...
	$str.=$opt;	
	$str.="<tr><td style=\"width:20%\"></td><td style=\"width:80%\"><input type=\"hidden\" value=\"{$f_id}\" name=\"f_id\"/><input type=\"submit\" name=\"submit\" class=\"pt_go\" value=\"提交\"/><input type=\"reset\" name=\"reset\" class=\"pt_reset\" value=\"重置\"/></td></tr>";
	$str.="</table></form>";
	}
	return $str;
}

function text($f_name,$f_value){
	$str="<input name=fields[{$f_name}] id=\"{$f_name}\" style=\"width:50%\" value=\"{$f_value}\" />";
	return $str;
}
function textarea($f_name,$f_value){
	$str="<textarea name=\"fields[{$f_name}]\" id=\"{$f_name}\" style=\"width:50%; height:50px;\">{$f_value}</textarea>";
	return $str;
}
function html($f_name,$f_value){
	 $fck=new FCKeditor("fields[".$f_name."]");
...
function upload_pic($f_name,$f_value){
	$str="<input name=\"fields[{$f_name}]\" value=\"{$f_value}\" style=\"width:30%;\" id=\"{$f_name}\"/>";
		$str.='<input style="margin-left:5px;" type="button" name="button" value="上传图片" onclick="window.open(\'upload.php?up_type=pic&get='.$f_name.'&lang='.$GLOBALS['lang'].'\',\'图片上传\',\'width=600,height=200\');"/>';
	return $str;
}
//多图上传
function upload_pic_more($f_name,$f_value){
    $pic="";
	$str="";
	if(!empty($f_value)){$value=preg_split('/,/',$f_value);$n=count($value);}
	if(!empty($value)){
	$i=0;
	foreach($value as $k=>$v){
	if(empty($v)){continue;}
	if($n==$i){break;}
	$pic.="<li id=\"pic_{$k}\"><a href=\"".CMS_SELF."upload/".$v."\" target=\"_blank\"><img src=\"".CMS_SELF."upload/".$v."\" border=\"0\" height=\"120\" width=\"120\"/><input type=\"hidden\" name=\"fields[{$f_name}][]\" value=\"".$v."\"/></a><span onclick=\"javascript:del_pic('".$v."',this);\">删除</span></li>";
	$i=$i+1;
	}
	}
	
		$str='<p><input style="margin-left:5px;" type="button" name="button" value="上传多张图片" onclick="window.open(\'upload_more.php?get='.$f_name.'&lang='.$GLOBALS['lang'].'\',\'图片上传多图\',\'width=600,height=200\');"/></p>';
		$str.="<ul id=\"show_pic\">{$pic}</ul>";
	return $str;
}
function upload_file($f_name,$f_value){
	$str="<input name=\"fields[{$f_name}]\" value=\"{$f_value}\" style=\"width:30%;\" id=\"{$f_name}\"/>";
		$str.='<input style="margin-left:5px;" type="button" name="button" value="上传文件" onclick="window.open(\'upload.php?up_type=file&get='.$f_name.'&lang='.$GLOBALS['lang'].'\',\'文件上传\',\'width=600,height=200\');"/>';
	return $str;
}
...
function get_post_catelist($lang,$channel,$cate_id=''){
	...
	if(!empty($cate)){
		...
		if($v['cate_channel']==$channel){
			echo "<option ";
			if($v['cate_tpl']==1){
				echo "value=\"index\">{$v['cate_name']}(频道栏目,不能发布内容)";
			}elseif($v['cate_hide']){
...
function up_img($file,$size,$type,$thumb=0,$thumb_width='',$thumb_height='',$logo=1){
		...
		$file_name=$path.date('YmdHis').'.'.$pic_name['extension'];
		$file_name=(!$logo)?CMS_PATH."upload/mark_logo.".$pic_name['extension']:$file_name;
		unset($pic_name);
		//开始上传
		...
		if($_sys['image_is'][0]&&$logo){		
			...
			if(!$_sys['image_type'][0]){
				...
				$font=DATA_PATH."font/simhei.ttf";
		...
		if($thumb){
			...
			switch($file_info[2]){
			case 1:
			imagegif($new_img,$path.date('YmdHis').'_thumb.gif');
			$return_name['thumb']=str_replace(CMS_PATH."upload/","",$path.date('YmdHis').'_thumb.gif');
			break;
			case 2:
			imagejpeg($new_img,$path.date('YmdHis').'_thumb.jpeg');
			$return_name['thumb']=str_replace(CMS_PATH."upload/","",$path.date('YmdHis').'_thumb.jpeg');
			break;
			case 3:
			imagepng($new_img,$path.date('YmdHis').'_thumb.png');
			$return_name['thumb']=str_replace(CMS_PATH."upload/","",$path.date('YmdHis').'_thumb.png');
			break;
			}
		}
...
function up_file($file,$size,$type,$path='',$name=''){
		...
		if(empty($type)){
			$type=array('rar','html','doc','zip','gz','iso','xsl','ppt','wps');
		}
...
function page($url='',$page,$query='',$totalnum,$totalpages,$cate_id='',$ishtml=0){
	...
	if($ishtml){
		...
		$str.='<li><a href="'.CMS_URL.'htm/'.$cate_info['cate_fold_name'].'/index.html">'.$language['pagehome'].'</a></li>';
		if($page>1){
			
			if(($page-1)==1){
				$str.='<li><a href="'.CMS_URL.'htm/'.$cate_info['cate_fold_name'].'/index.html">'.$language['pagapre'].'</a></li>';
			}else{
				$str.='<li><a href="'.CMS_URL.'htm/'.$cate_info['cate_fold_name'].'/list_'.$pre_page.'.html">'.$language['pagapre'].'</a></li>';
			}
		}else{
			$str.='<li><a href="#">'.$language['pagapre'].'</a></li>';
		}
		for($i=1;$i<=$totalpages;$i++){
			$class=($page==$i)?"focus":"";
			$sel=($page==$i)?"selected=\"selected\"":"";
			if($i==1){
				$str.='<li class="'.$class.'"><a href="'.CMS_URL.'htm/'.$cate_info['cate_fold_name'].'/index.html">1</a></li>';
				$op.="<option value=\"index.html\" {$sel}>{$i}</option>";
			}else{
			$str.='<li class="'.$class.'"><a href="'.CMS_URL.'htm/'.$cate_info['cate_fold_name'].'/list_'.$i.'.html">'.$i.'</a></li>';
			$op.="<option value=\"list_{$i}.html\" {$sel}>{$i}</option>";
			}
			
			
		}
		if($page<$totalpages){
			$str.='<li><a href="'.CMS_URL.'htm/'.$cate_info['cate_fold_name'].'/list_'.$next_page.'.html">'.$language['pagenext'].'</a></li>';
		}else{
			$str.='<li><a href="#">'.$language['pagenext'].'</a></li>';
		}
		$str.='<li><a href="'.CMS_URL.'htm/'.$cate_info['cate_fold_name'].'/list_'.$totalpages.'.html">'.$language['pageend'].'</a></li>';
		$str.="{$language['pagego']}<select style=\"width:40px;\" onchange=\"location.href=this.options[this.selectedIndex].value;\">".$op."</select>";
		$str.="<li>{$language['pages']}{$totalnum}{$language['pagesize']},{$language['page']}{$page}/{$totalpages}</li>";
	}else{
		$str.='<li><a href="'.$url.'?page=1'.$query.'">'.$language['pagehome'].'</a></li>';
		if($page>1){
			$str.='<li><a href="'.$url.'?page='.$pre_page.$query.'">'.$language['pagapre'].'</a></li>';
		}else{
			$str.='<li><a href="#">'.$language['pagapre'].'</a></li>';
		}
		for($i=1;$i<=$totalpages;$i++){
			$class=($page==$i)?"focus":"";
			if($i==1){
				$str.='<li class="'.$class.'"><a href="'.$url.'?page='.$i.$query.'">1</a></li>';
			}else{
				$str.='<li class="'.$class.'"><a href="'.$url.'?page='.$i.$query.'">'.$i.'</a></li>';
			}
			$sel=($page==$i)?"selected=\"selected\"":"";
			$op.="<option value=\"?page={$i}{$query}\" {$sel}>{$i}</option>";
		}
		if($page<$totalpages){
			$str.='<li><a href="'.$url.'?page='.$next_page.$query.'">'.$language['pagenext'].'</a></li>';
		}else{
			$str.='<li><a href="#">'.$language['pagenext'].'</a></li>';
		}
		$str.='<li><a href="'.$url.'?page='.$totalpages.$query.'">'.$language['pageend'].'</a></li>';
		$str.="<li>{$language['pagego']}<select style=\"width:40px;\" onchange=\"location.href=this.options[this.selectedIndex].value;\">".$op."</select></li>";
		$str.="<li>{$language['pages']}{$totalnum}{$language['pagesize']},{$language['page']}{$page}/{$totalpages}</li>";
	}
	return $str;
}
...
function is_login(){
	...
	if($_SESSION['login_in']==1&&!empty($_SESSION['admin'])){
		if(mktime()-$_SESSION['login_time']>1800){
			login_out();
		}else{
		$_SESSION['login_time']=mktime();
		}
...
function login_out(){
	...
	if(isset($_SESSION['admin_ip'])&&isset($_SESSION['admin_time'])&&isset($_SESSION['admin_id'])){
	...
	$_SESSION['admin_time']=array();
	$_SESSION['admin_ip']=array();
...
function admin_purview(){
	...
	foreach($admin_group as $k=>$v){
	    $rel=0;
		if(intval($v['id'])==$admin&&$v['is_disable']==0){
...
function get_cate_list($cate,$parent,$haschild,$level=0){
		...
		$level=$level+1;
			foreach($cate as $k=>$v){
				...
				echo "<div id=\"border\" style=\"border-bottom:1px dashed #ccc; padding:2px 0;\"><div class=\"left\" id=\"show\" style=\"padding-left:".$pd."px; cursor:pointer\"><span class=\"exp\" onclick=\"show_list(this);\">&nbsp;</span><span class=\"cata\"><a href=\"".CMS_SELF."show_list.php?id={$v['id']}\" target=\"_blank\">".$v['cate_name']."</a>(<span style=\"color:#999\">排序</span>".$v['cate_order']."&nbsp;<span style=\"color:#999\">模板id</span>:".$v['id'].")";
...
//添加内容生成html地址
function get_ct_path($addtime,$cate_id,$last_id){
	if(file_exists(DATA_PATH.'cache_cate/cache_category_all.php')){
		include(DATA_PATH.'cache_cate/cache_category_all.php');
	}
	$cate_info=get_cate_info($cate_id,$category);
	...
	return 'htm/'.$cate_info['cate_fold_name'].'/'.$addtime_rel[0].'/'.$addtime_rel[1].$addtime_rel[2].'/'.$last_id.'.html';

}
//取得位置
function get_position($parent,$cate,$path){
	global $_confing;
	$str='';
	foreach($cate as $k=>$v){
		if($v['id']==$parent){
			$cate_path=($v['cate_html']&&$_confing['web_html'][0])?$path.'htm/'.$v['cate_fold_name']:$path.'show_list.php?id='.$v['id'];
			$str="<a href=\"{$cate_path}\">{$v['cate_name']}</a> > ";
			get_position($v['cate_parent'],$cate,$path);
			break;
		}
	}
	echo $str;
...
function get_child_id($id){
	$arr='';
	$sql="select id from ".DB_PRE."category where cate_parent=".intval($id);
...
function get_sitemap_child($id){
	...
	if(!empty($child)){
		foreach($child as $row){
			$url=($row['cate_html']&&$GLOBALS['_confing']['web_html'][0])?$path.'htm/'.$row['cate_fold_name']:$path.'show_list.php?id='.$row['id'];
					if($row['cate_tpl']==3){
					$url=($row['cate_html']&&$GLOBALS['_confing']['web_html'][0])?$path.'htm/'.$row['cate_fold_name'].'/index_'.$lang.'.html':$path.'show_list.php?id='.$row['id'];}
			$rel[$row['id']]['url']=$url;
...
//获得下级栏目
function get_child_cate($parent_id,$lang){
	if(empty($parent_id)){return;}
	$child=array();
	$_confing=get_confing($lang);
	$sql="select*from ".DB_PRE."category where cate_parent=".$parent_id;
	...
	foreach($rel as $row){
		if(!$row['cate_hide']){
			if($row['cate_tpl']==2){
				...
				$url=($row['cate_html']&&$_confing['web_html'][0])?CMS_SELF.'htm/'.$row['cate_fold_name']:CMS_SELF.'show_list.php?id='.$row['id'];
			if($row['cate_tpl']==3){
				$url=($row['cate_html']&&$_confing['web_html'][0])?CMS_SELF.'htm/'.$row['cate_fold_name'].'/index_'.$lang.'.html':CMS_SELF.'show_list.php?id='.$row['id'];
			}
			}
