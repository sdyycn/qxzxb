...
class limidb {
    ...
    *******************************************************************/
    ...
    /******************************************************************
    -- 函数名：get_result($Table,$Fileds,$Condition,$Rows)
    -- 作  用：根据条件查询数据
    -- 参  数：$Table 表名(必填)
              $Fileds 字段名，默认为所有(选填)
              $Condition 查询条件(选填)
    ...
    *******************************************************************/
    public function get_result($Table, $Fileds = '*', $Condition = '', $Rows = 0) {
        if (!$Fileds) $Fileds = '*';
    ...
    --返回值：多维数组
    * */
    function fetch_result($result, $field = null) {
        $ret_array = array();
    ...
    /******************************************************************
    -- 函数名：get_one($Table,$Fileds,$Condition)
    -- 作  用：查询符合条件的一条数据
    -- 参  数：$Table 表名(必填)
              $Fileds 字段名，默认为所有(选填)
              $Condition 查询条件(选填)
    ...
    -- 实  例：$DB->Get('mydb','user,password','order by id desc',10)
    *******************************************************************/
    public function get_one($Table, $Fileds = '*', $Condition = '') {
        if (!$Fileds) $Fileds = '*';
        $this->query("SELECT {$Fileds} FROM `{$Table}` {$Condition} LIMIT 0,1");
        return $this->fetch_array();
    }
    /******************************************************************
    -- 函数名：insert($Table,$Data)
    -- 作  用：添加数据
    -- 参  数：$Table 表名(必填)
              $Data 待添加数据,可以为数组(必填)
    -- 返回值：布尔
    -- 实  例：$DB->insert('mydb',array('user'=>'admin','password'=>'123456','age'=>'18') 数组类型
              $DB->insert('mydb','user=admin,password=123456,age=18') 字符串类型
    *******************************************************************/
    ...
    /******************************************************************
    -- 函数名：update($Table,$Data,$Condition,$unQuot)
    -- 作  用：更改数据
    -- 参  数：$Table 表名(必填)
              $Data 待更改数据,可以为数组(必填)
              $Condition 更改条件(选填)
    ...
    *******************************************************************/
    public function update($Table, $Data, $Condition = '', $unQuot = '') {
        if (is_array($Data)) {
    ...
    *******************************************************************/
    ...
    /******************************************************************
    -- 函数名：result($result)
    -- 作  用：返回结果集中的第一行第一个字段的值
    -- 参  数：$result  结果记录集
    -- 返回值：布尔
    -- 实  例：$DB->result($result)
    *******************************************************************/
    ...
    *******************************************************************/
    ...
    // 获取关联数组,使用$row['字段名'],从结果集中取得一行
    public function fetch_assoc($result = '') {
        if (empty($result)) $result = $this->result;
        if (!$result) $this->show_error('未获取到查询结果', true);
        return mysql_fetch_assoc($result);
    }
    // 获取数字索引数组,使用$row[0],$row[1],$row[2]，从结果集汇总取得一行
        ...
        if ($this->pageAll > 1) {
            ...
            if ($number) {
                ...
                if ($this->pageAll < 6) {
                    ...
                    elseif ($this->pageNo <= $this->pageAll && $this->pageNo > ($this->pageAll - 3)) for ($i = 1; $i < 6; $i++) $arr[] = $this->pageAll - 5 + $i;
                    else for ($i = 1; $i < 6; $i++) $arr[] = $this->pageNo - 3 + $i;
                }
                foreach ($arr as $val) {
