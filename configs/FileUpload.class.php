...
class FileUpload {
	...
	private $dest_name;
	private $size;
	private $maxsize = 10240;	// 10M
	private $error;
	private $type;
	private $limit_type = array('gif', 'jpg', 'png', 'jpeg');
	private $filepath;
	private $msg;
	...
	function upload($file, $path) {
		...
		if (!$this->checkSize()){
			$ret = -1;
			$this->msg = "checkSize failed.";
		} else if (!$this->checkType()){
			$ret = -2;
			$this->msg = "checkType failed.";
		} else if (!$this->checkPath()){
			$ret = -3;
			$this->msg = "checkPath failed.";
		} else if (!$this->checkName()){
			$ret = -4;
			$this->msg = "checkName failed.";
		} else if (!$this->copyFile()){
			$ret = -5;
			$this->msg = "copyFile failed.";
		}
	...
	private function checkName() {
		if (count($this->src_name)<1) {
			return false;
		}
		
		$name = date("YmdHis").'.'.$this->type;
		$file = $this->path. '/'. $name;
		while (file_exists($file)) {
			$name = date("YmdHis").rand(100,999).'.'.$this->type;
			$file = $this->path. '/'. $name;
		}
	...
	private function checkType() {
		$arrStr = explode('.', $this->src_name);
		$type = strtolower($arrStr[count($arrStr)-1]);
		
//		if (!in_array($type, $this->limit_type)) {
//			return false;
//		}
	...
	private function copyFile(){
		$filepath = $this->path . '/'. $this->dest_name;
 ...
 * @return    新文件名
**/
private function FILE_get_upload_filename($upload_name, $dest_path)
{
